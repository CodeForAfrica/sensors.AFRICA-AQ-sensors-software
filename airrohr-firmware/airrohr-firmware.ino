...
namespace cfg {
  ...
  char basic_authentication_influx[BASIC_AUTHENTICATION_INFLUX];
    ...
    strcpy_P(basic_auth_influx, BASIC_AUTHENTICATION_INFLUX);
 ...
 *****************************************************************/
#if defined(ESP8266)
SoftwareSerial serialSDS(SDS_PIN_TX, SDS_PIN_RX, false, 128);
...
static void webserver_config_send_body_get(String& page_content) {
  ...
  add_form_chechbox(Config_gsm_capable, FPSTR(INTL_GSM_ATTACHED));
...
void loop(void) {
	...
	if (send_now) {
		...
		if (cfg::ppd_read) {
			data += result_PPD;
      sum_send_time += sendCFA(result_PPD, PPD_API_PIN, FPSTR(SENSORS_PPD4NS), "PPD_");
    ...
    if (cfg::sps30_read && (! sps30_init_failed)) {
      ...
      data += result;
      sum_send_data += sendCFA(result, SPS30_API_PIN, FPSTR(SENSORS_SPS30), "SPS30_");
